...
async function seed() {
    try {
        ...
        await Candidate_1.CandidateModel.deleteMany({});
        await CandidatePositions_1.CandidatePositionsModel.deleteMany({});
        await Manifesto_1.ManifestoModel.deleteMany({});
        console.log('Seeding 4 manifestos...');
        await Manifesto_1.ManifestoModel.insertMany(MANIFESTOS);
        console.log('Seeding 12 candidates...');
        for (const data of CANDIDATES_DATA) {
            const { scores, ...candidateData } = data;
            const candidate = await Candidate_1.CandidateModel.create(candidateData);
            await CandidatePositions_1.CandidatePositionsModel.create({
